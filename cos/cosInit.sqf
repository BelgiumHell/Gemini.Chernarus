...
blackListTowns = ["sagonisi","triadi","ekali"];// Remove towns from COS
...
COSmaxGrps=1;//Set Maximum group limit for COS at any one time (If limit is hit then civilians will be placed into a single group for each town)
...
_randomisation=7;
	...
	_parked=2;// Parked Vehicles Spawned
 	...
 	if (_houses <= 140 and _houses > 70)
		then {
	_civilians=28+ round(random _randomisation);// Civilians spawned
