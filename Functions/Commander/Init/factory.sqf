...
/////////////////////////
_usedCompounds = [];
_diesel = (getMarkerPos "mrk_area") nearEntities [["Land_dp_mainFactory_F"],worldSize*2.0^0.5];
_main = (getMarkerPos "mrk_area") nearEntities [["Land_Factory_Main_F"],worldSize*2.0^0.5];

{
    if(!(_x in _usedCompounds))then{
        ...
        _compounds = (getPos _x) nearEntities [["Land_Factory_Main_F"],200];
...

{
    if(!(_x in _usedCompounds))then{
        ...
        _compounds = (getPos _x) nearEntities [["Land_dp_mainFactory_F"],200];
        _usedCompounds append _compounds;
