...
[[[_truck,_objArray,_name],{_action = (_this select 0) addAction["Dismantle FOB",{[(_this select 0),(_this select 3 select 0)] spawn JOC_fobUndeploy; deleteMarker (_this select 3 select 1);},[(_this select 1),(_this select 2)],1,true,true,"","(side _this == west) && (speed _target < 1)"];}],"BIS_fnc_spawn",true,true] call BIS_fnc_MP;
